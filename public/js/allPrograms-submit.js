let retriveProgramsInCoursesForm = async () => {
    const url = "http://localhost:3000/allPrograms";
    await fetch(url)
        .then(res => res.json())
...
let retriveProgramsInProgramHTML = async () => {
    ...
    if (jsonData.length == 0) {
        ...
        jsonData.forEach((item) => {
            ...
            // cardHTML +='<div class="col-md-4" style="float:left"><div class="card mb-2"><img class="card-img-top mx-auto d-block" style="width:100%" src="'
            // item.imageURL +
            // '" alt="Card image cap"><div class="card-body"><h4 class="card-title">'+
            // item.title +
            // '</h4><p class="card-text">'+
